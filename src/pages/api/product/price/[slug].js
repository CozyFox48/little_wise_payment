...
const handler = async (req, res) => {
    ...
    if (req.method === 'POST') {
        try {
            ...
            await Product.findByIdAndUpdate(slug, { $push: { prices: requestBody } });
        ...
        try {
            const { slug } = req.query;
            const requestBody = req.body.newPrices;
            await Product.findByIdAndUpdate(slug, { $set: { prices: requestBody } });
            const _product = await Product.findById(slug).populate('business');
